...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        times = len(self.place.bees)
        i = 0
        while i < times:
            if bees[i].health - damage_dealt <= 0:
                bees[i].reduce_health(damage_dealt)
                times -= 1
            else:
                bees[i].reduce_health(damage_dealt)
                i += 1
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.scared_remaining:
            if not (self.slow_remaining and gamestate.time % 2 != 0):
                self.scared_remaining -= 1
                if not self.place.is_hive:
                    destination = self.place.entrance

        if self.slow_remaining:
